    ...
    for i in range(len(sequences[seqnames[0]])):
        ...
        for seq in sequences:
            if sequences[seq][i]  == 'A':
                ...
                T += 1
        # divide nucleotide counts by the number of sequence
        profile['A'].append(round(A/len(sequences), 4))
        profile['T'].append(round(T/len(sequences), 4))
        profile['C'].append(round(C/len(sequences), 4))    
        profile['G'].append(round(G/len(sequences), 4))    
...
def greedy_motif_search(input_file):
    '''
    (file) -> dict
